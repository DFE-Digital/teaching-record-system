    ...
    EvidenceUploadManager evidenceUploadManager) : JourneyCoordinator<MergePersonState>
{
    ...
    /// <summary>
    /// Gets the records identified by <paramref name="personIds"/>, in the order they were asked for,
    /// labelled A, B, … and with the attributes each shares with the first record.
    /// </summary>
    ...
    /// <summary>
    /// Gets the attributes whose values match those on <paramref name="recordToMatchAgainst"/>.
    /// </summary>
    public static IReadOnlyCollection<PersonMatchedAttribute> GetPersonAttributeMatches(
        ...
        Gender? gender)
    {
        ArgumentNullException.ThrowIfNull(recordToMatchAgainst);
