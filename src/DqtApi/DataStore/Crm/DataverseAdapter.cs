    ...
    public partial class DataverseAdapter : IDataverseAdapter
    {
        ...
        public async Task<IReadOnlyCollection<Contact>> FindTeachers(FindTeachersQuery filter)
        {
            var fields = new[]
            {
                        ...
                        (FieldName: Contact.Fields.dfeta_NINumber, Value: filter.Nino),
                        (FieldName: Contact.Fields.EMailAddress1, Value: filter.EmailAddress),
                        (FieldName: Account.PrimaryIdAttribute, Value: filter.IttProviderOrganizationId),
            ...
            if (filter.IttProviderOrganizationId.HasValue)
            {
                ...
                le1.Columns = new ColumnSet(true);
