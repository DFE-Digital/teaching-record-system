...
public class PersonService(TrsDbContext dbContext, OneLoginService oneLoginService, IEventPublisher eventPublisher)
{
    public Task<Person?> GetPersonAsync(Guid personId, bool includeDeactivatedPersons = false)
    {
        ...
        {
            persons = persons.IgnoreQueryFilters([Person.QueryFilterNames.Deactivated]);
        }
    ...
    {
        var person = await dbContext.Persons.IgnoreQueryFilters([Person.QueryFilterNames.Deactivated]).SingleOrDefaultAsync(p => p.PersonId == options.PersonId);
    ...
    {
        var deactivatingPerson = await dbContext.Persons.IgnoreQueryFilters([Person.QueryFilterNames.Deactivated]).SingleOrDefaultAsync(p => p.PersonId == options.PersonId);
    ...
    {
        var reactivatingPerson = await dbContext.Persons.IgnoreQueryFilters([Person.QueryFilterNames.Deactivated]).SingleOrDefaultAsync(p => p.PersonId == personId);
    ...
    {
        var deactivatingPerson = await dbContext.Persons.IgnoreQueryFilters([Person.QueryFilterNames.Deactivated]).SingleOrDefaultAsync(p => p.PersonId == options.DeactivatingPersonId);
        ...
        var retainedPerson = await dbContext.Persons.IgnoreQueryFilters([Person.QueryFilterNames.Deactivated]).SingleOrDefaultAsync(p => p.PersonId == options.RetainedPersonId);
