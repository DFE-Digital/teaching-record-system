...
public class CapitaExportMissingTrnJob([FromKeyedServices("sftpstorage")] DataLakeServiceClient dataLakeServiceClient, ILogger<CapitaExportMissingTrnJob> logger, TrsDbContext dbContext, TimeProvider timeProvider)
{
    public const string Trn = "0741653";
    ...
    {
        var person = await dbContext.Persons.SingleAsync(p => p.Trn == Trn, cancellationToken);
        ...
        {
            //write rows
            foreach (var personToExport in new[] { person })
            {
                var hasFailed = false;
                ...
                try
                {
                    ...
                    previousNameRow = await GetNewPersonWithPreviousLastNameAsStringRowAsync(personToExport, cancellationToken);
                    var hasPreviousLastName = !string.IsNullOrEmpty(previousNameRow);

                    // write current name row
                    row = GetNewPersonAsStringRow(personToExport, hasPreviousLastName);
                ...
                integrationJob.IntegrationTransactionRecords.Add(new IntegrationTransactionRecord()
                {
                    ...
                    PersonId = personToExport.PersonId,
