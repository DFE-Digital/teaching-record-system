...
public enum SupportTaskType
{
    ...
    [SupportTaskTypeDescription("TRN request from NPQ", SupportTaskCategory.TrnRequests, typeof(NpqTrnRequestData), legacy: true)]
    NpqTrnRequest = 5,
...
public static class SupportTaskTypeRegistry
{
    ...
    public static IReadOnlyCollection<SupportTaskTypeDescription> GetAll(bool includeLegacy = true) =>
        _info.Values.Where(t => !t.Legacy || includeLegacy).OrderBy(s => s.Title).ToArray();
    ...
    private static SupportTaskTypeDescription GetInfo(SupportTaskType supportTaskType)
    {
        ...
        return new SupportTaskTypeDescription(supportTaskType, attr.Name, attr.SupportTaskCategory, attr.DataType, attr.Legacy);
    }
}
...
file sealed class SupportTaskTypeDescriptionAttribute(string name, SupportTaskCategory category, Type dataType, bool legacy = false) : Attribute
{
