CREATE OR REPLACE FUNCTION fn_split_names(
    ...
    WITH normalized_names AS (
        SELECT translate(
            coalesce(n, ''),
            U&'\0060\00B4\2018\2019',  -- grave, acute, left single quote, right single quote
            U&'\0027\0027\0027\0027'   -- normalize to: apostrophe (U+0027) 4 times
        ) AS normalized_name
        FROM unnest(names) AS data(n)
    ),
    split_names AS (
        SELECT unnest(string_to_array(regexp_replace(trim(normalized_name), '(\s|-)+', ' '), ' ')) name_part, false is_synonym
