...
public class TrnRequestTests(HostFixture hostFixture) : TestBase(hostFixture)
{
    [Fact]
    public async Task Resolve_CreateNewRecord()
    {
        ...
        await WithDbContextAsync(dbContext =>
            dbContext.SupportTasks.Where(t => t.SupportTaskType == SupportTaskType.TrnRequest).ExecuteDeleteAsync());

        var applicationUser = await TestData.CreateApplicationUserAsync();
        var (supportTask, requestData, matchedPersonIds) = await TestData.CreateTrnRequestSupportTaskAsync(applicationUser.UserId, t => t.WithStatus(SupportTaskStatus.Open));
        ...
        await page.WaitForUrlPathAsync($"/support-tasks/trn-requests/{supportTask.SupportTaskReference}/resolve/check-answers");
        await page.ClickGovUkButtonAsync("Confirm and create record");

        await page.WaitForUrlPathAsync("/support-tasks/trn-requests");
    ...
    public async Task Resolve_MultiplePotentialMatches_MergeWithExistingRecord()
    {
        ...
        var (supportTask, requestData, _) = await TestData.CreateTrnRequestSupportTaskAsync(
        ...
        await page.WaitForUrlPathAsync($"/support-tasks/trn-requests/{supportTask.SupportTaskReference}/resolve/merge");
        ...
        await page.WaitForUrlPathAsync($"/support-tasks/trn-requests/{supportTask.SupportTaskReference}/resolve/check-answers");
        await page.ClickGovUkButtonAsync("Confirm and update existing record");
    ...
    public async Task Resolve_NoLongerAnyPotentialMatches_CreateNewRecord()
    {
        ...
        await page.WaitForUrlPathAsync($"/support-tasks/trn-requests/{supportTask.SupportTaskReference}/resolve/matches");
        await page.ClickGovUkButtonAsync("Create a record from this request");
    ...
    public async Task Resolve_DefiniteMatchNowFound_MergeWithExistingRecord()
    {
        ...
        await page.WaitForUrlPathAsync($"/support-tasks/trn-requests/{supportTask.SupportTaskReference}/resolve/matches");
        await page.ClickGovUkButtonAsync("Merge this request with Record A");
