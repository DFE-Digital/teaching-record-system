...
public class AlertDetailsTests(HostFixture hostFixture) : TestBase(hostFixture), IAsyncLifetime
{
    ...
    public async Task Get_ValidRequest_RendersExpectedContent()
    {
        ...
        Assert.Equal(alert.StartDate?.ToString(WebConstants.DateDisplayFormat), doc.GetSummaryListValueByKey("Start date"));
        Assert.Equal(alert.EndDate?.ToString(WebConstants.DateDisplayFormat), doc.GetSummaryListValueByKey("End date"));
    }
