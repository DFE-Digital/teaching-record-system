...
public class ChangeHistoryEntryViewTestController(TrsDbContext dbContext, ChangeHistoryService changeHistoryService) : Controller
{
    ...
    public async Task<IActionResult> ChangeHistoryEntry(Guid processId, [FromQuery] Guid? personId, [FromQuery] string? contextType, [FromQuery] string? oneLoginSubject, [FromQuery] string? supportTaskReference)
    {
        ...
        var contextData = await changeHistoryService.GetContextDataAsync([process]);

        var vm = new ChangeHistoryEntryViewModel
        {
            Context = contextType switch
            {
                "oneLogin" => ChangeHistoryContext.ForOneLoginUser(oneLoginSubject ?? string.Empty, contextData.AllPersons, contextData.AllOneLoginUsers),
                "supportTask" => ChangeHistoryContext.ForSupportTask(supportTaskReference ?? string.Empty, contextData.AllPersons, contextData.AllOneLoginUsers),
                _ => ChangeHistoryContext.ForPerson(personId ?? Guid.Empty, contextData.AllPersons, contextData.AllOneLoginUsers),
            },
