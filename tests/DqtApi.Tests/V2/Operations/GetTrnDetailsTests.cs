...
namespace DqtApi.Tests.V2.Operations
{
    public class GetTrnDetailsTests : ApiTestBase
    {
        ...
        [Fact]
        public async Task When_no_results_found_then_response_is_ok()
        {
            // Arrange
            var request = new HttpRequestMessage(HttpMethod.Get, $"v2/trn-details/trn-details-match");
        ...
        public async Task Given_SearchParameters_SearchResults_Match()
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Get, $"v2/trn-details/trn-details-match?FirstName={contact1.FirstName}&LastName={contact1.LastName}");
                ...
                expected: new
                {
                    details = new[]
                    {
                        new
                        {
                             ...
                             dateOfBirth = contact1.BirthDate?.ToString(),
                             nino = contact1.dfeta_NINumber,
        ...
        public async Task Given_NonExistent_ProviderNameOrUkprnProvider_ReturnError(string providerName, string providerUkprn)
        {
            // Arrange
            var contact1 = new Contact() { FirstName = "test", LastName = "testing", Id = Guid.NewGuid(), dfeta_NINumber = "1111", BirthDate = new DateTime(1988, 1, 1), dfeta_TRN = "someReference" };
            var request = new HttpRequestMessage(HttpMethod.Get, $"v2/trn-details/trn-details-match?FirstName={contact1.FirstName}&LastName={contact1.LastName}&IttProviderUkPrn={providerUkprn}&IttProviderName={providerName}");
            ApiFixture.DataverseAdapter
                .Setup(mock => mock.FindTeachers(It.IsAny<FindTeachersQuery>()))
                .ReturnsAsync(new List<Contact> { contact1 })
                .Verifiable();
        ...
        public async Task Given_BothUkPrnAndProviderNameAreProvided_ReturnError()
        {
            // Arrange
            var contact1 = new Contact() { FirstName = "test", LastName = "testing", Id = Guid.NewGuid(), dfeta_NINumber = "1111", BirthDate = new DateTime(1988, 1, 1), dfeta_TRN = "someReference" };
            var request = new HttpRequestMessage(HttpMethod.Get, $"v2/trn-details/trn-details-match?FirstName={contact1.FirstName}&LastName={contact1.LastName}&IttProviderUkPrn=12345678910&IttProviderName=provider");
            ApiFixture.DataverseAdapter
                .Setup(mock => mock.FindTeachers(It.IsAny<FindTeachersQuery>()))
                .ReturnsAsync(new List<Contact> { contact1 })
                .Verifiable();
