...
public class ContinueToApplicationTests(HostFixture hostFixture) : TestBase(hostFixture)
{
    [Fact]
    public Task Get_ValidRequest_RendersExpectedContent() =>
        WithJourneyCoordinatorAsync(
            ...
            async coordinator =>
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Get, JourneyUrls.ContinueToApplication(coordinator.InstanceId));
    ...
    public Task Post_ValidRequest_RedirectsToStateRedirectUri() =>
        WithJourneyCoordinatorAsync(
            ...
            async coordinator =>
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Post, JourneyUrls.ContinueToApplication(coordinator.InstanceId));
        ...
        string nationalInsuranceNumber)
    {
        ...
        AddUrlToPath(coordinator, StepUrls.ContinueToApplication);
    }
}
