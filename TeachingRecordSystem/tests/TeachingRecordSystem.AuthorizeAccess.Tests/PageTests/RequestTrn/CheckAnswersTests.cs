...
public class CheckAnswersTests(HostFixture hostFixture) : TestBase(hostFixture), IAsyncLifetime
{
    ...
    public async Task Get_ValidRequest_PopulatesModelFromJourneyState(bool hasNationalInsuranceNumber)
    {
        ...
        Assert.Equal(' '.JoinNonEmpty(state.FirstName, state.MiddleName, state.LastName), doc.GetSummaryListValueByKey("Name"));
        Assert.Equal(' '.JoinNonEmpty(state.PreviousFirstName, state.PreviousMiddleName, state.PreviousLastName), doc.GetSummaryListValueByKey("Previous name"));
