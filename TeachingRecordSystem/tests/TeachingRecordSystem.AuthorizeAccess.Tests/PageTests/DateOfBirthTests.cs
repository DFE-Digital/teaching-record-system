    ...
    public Task Get_ValidRequest_ReturnsExpectedContent(bool haveExistingValueInState) =>
        WithJourneyCoordinatorAsync(
            ...
            async coordinator =>
            {
                ...
                {
                    Assert.Equal("", doc.GetElementById("DateOfBirth.Day")?.GetAttribute("value"));
                    Assert.Equal("", doc.GetElementById("DateOfBirth.Month")?.GetAttribute("value"));
                    Assert.Equal("", doc.GetElementById("DateOfBirth.Year")?.GetAttribute("value"));
                }
            });
