...
public partial class SetQtlsTests(OperationTestFixture operationTestFixture) : OperationTestBase(operationTestFixture)
{
    ...
    private Task<ProfessionalStatus?> GetQtlsRoute(Guid personId) =>
        DbFixture.WithDbContextAsync(dbContext =>
            dbContext.Qualifications
                .OfType<ProfessionalStatus>()
                .IgnoreQueryFilters()
                .SingleOrDefaultAsync(q => q.PersonId == personId && q.RouteToProfessionalStatusId == RouteToProfessionalStatus.QtlsAndSetMembershipId));
}
