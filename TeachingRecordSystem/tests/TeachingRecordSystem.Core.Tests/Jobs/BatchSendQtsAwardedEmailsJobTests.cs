...
public class BatchSendQtsAwardedEmailsJobTests : QtsAwardedEmailJobTestBase
{
    ...
    public static TheoryData<DateTime, DateTime?, DateTime, DateTime, DateTime> DateRangeEvaluationTestData { get; } =
        new()
        {
    ...
    public Task Execute_WhenHasAwardeesForDateRange_UpdatesDatabaseAndEnqueuesJobToSendEmail() =>
        DbFixture.WithDbContextAsync(async dbContext =>
        {
            ...
            // Assert
            var jobItem =
                await dbContext.QtsAwardedEmailsJobItems.SingleOrDefaultAsync(i => i.PersonId == qtsAwardee1.TeacherId);
            ...
            backgroundJobScheduler
                .Verify(
                    s => s.EnqueueAsync(It
                        .IsAny<System.Linq.Expressions.Expression<Func<QtsAwardedEmailJobDispatcher, Task>>>()),
                    Times.Once);
        });
