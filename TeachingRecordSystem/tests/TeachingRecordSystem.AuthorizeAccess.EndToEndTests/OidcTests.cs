...
public class OidcTests(HostFixture hostFixture) : TestBase(hostFixture)
{
    ...
    public async Task RefreshToken_ExchangeForNewAccessToken_ReturnsNewTokens()
    {
        ...
        // Sign in and get initial tokens
        await page.GotoAsync("/oidc-test");
        await page.ClickButtonAsync("Start");
        await page.WaitForUrlPathAsync("/oidc-test/signed-in");
        ...
        var currentAccessBeforeExchange = await page.GetByLabel("Current access token").InputValueAsync();
        var currentRefreshBeforeExchange = await page.GetByLabel("Current refresh token").InputValueAsync();
        Assert.Equal(initialAccessToken, currentAccessBeforeExchange);
        Assert.Equal(initialRefreshToken, currentRefreshBeforeExchange);
