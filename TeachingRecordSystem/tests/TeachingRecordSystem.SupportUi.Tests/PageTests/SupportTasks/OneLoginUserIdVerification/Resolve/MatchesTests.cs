...
public class MatchesTests(HostFixture hostFixture) : ResolveOneLoginUserIdVerificationTestBase(hostFixture)
{
    ...
    public async Task Get_MatchedRecords_HighlightsNotMatchedFields()
    {
        ...
        Assert.Equal($"{matchedPerson3.PreviousNames.First().FirstName} {matchedPerson3.PreviousNames.First().LastName}", matchDetails.GetSummaryListValueByKey("Previous names"));
        Assert.Equal(UiDefaults.EmptyDisplayContent, matchDetails.GetSummaryListValueByKey("National Insurance number"));
        AssertMatchRowNotHighlighted(matchDetails, "Name");
        AssertMatchRowIsHighlighted(matchDetails, "National Insurance number");
    }
