...
public class ChangeLogMandatoryQualificationEventsTests : TestBase
{
    ...
    public async Task Person_WithMandatoryQualificationDeletedEvent_RendersExpectedContent()
    {
        ...
        Assert.Collection(
            ...
            item =>
            {
                ...
                Assert.Equal(mq.StartDate!.Value.ToString(UiDefaults.DefaultDateOnlyDisplayFormat), item.GetElementByTestId("start-date")?.TextContent.Trim());
                Assert.Equal(mq.Status!.Value.GetTitle(), item.GetElementByTestId("status")?.TextContent.Trim());
                Assert.Equal(mq.EndDate!.Value.ToString(UiDefaults.DefaultDateOnlyDisplayFormat), item.GetElementByTestId("end-date")?.TextContent.Trim());
            });
    }
    ...
    public async Task Person_WithMandatoryQualificationUpdatedEventWithChangedStartDate_RendersStartDateRowWithinPreviousData()
    {
        ...
        Assert.Collection(
            doc.GetAllElementsByTestId("timeline-item-mq-updated-event"),
            item => Assert.Equal(oldStartDate.ToString(UiDefaults.DefaultDateOnlyDisplayFormat), item.GetElementByTestId("start-date")?.TextContent.Trim()));
    }
    ...
    public async Task Person_WithMandatoryQualificationUpdatedEventWithChangedEndDate_RendersEndDateRowWithinPreviousData()
    {
        ...
        Assert.Collection(
            doc.GetAllElementsByTestId("timeline-item-mq-updated-event"),
            item => Assert.Equal(oldEndDate.ToString(UiDefaults.DefaultDateOnlyDisplayFormat), item.GetElementByTestId("end-date")?.TextContent.Trim()));
    }
