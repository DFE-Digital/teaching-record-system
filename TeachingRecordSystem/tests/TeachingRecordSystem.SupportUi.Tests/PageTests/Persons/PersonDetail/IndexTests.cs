...
public class IndexTests : TestBase
{
    ...
    public async Task Get_WithPersonIdForExistingPersonWithAllPropertiesSet_ReturnsExpectedContent()
    {
        ...
        var previousFirstNameChangedOn = new DateOnly(2021, 01, 01);
        ...
        var createPersonResult = await TestData.CreatePerson(
            b => b.WithEmail(email)
             ...
             .WithPreviousFirstName(previousFirstName, previousFirstNameChangedOn)
             .WithPreviousMiddleName(previousMiddleName, previousMiddleNameChangedOn)
        ...
        Assert.Equal($"{createPersonResult.FirstName} {createPersonResult.PreviousNames[1].Name} {createPersonResult.LastName}", summaryList.GetElementByTestId("personal-details-previous-names-0")!.TextContent);
        Assert.Equal($"{createPersonResult.PreviousNames[0].Name} {createPersonResult.PreviousNames[1].Name} {createPersonResult.LastName}", summaryList.GetElementByTestId("personal-details-previous-names-1")!.TextContent);
