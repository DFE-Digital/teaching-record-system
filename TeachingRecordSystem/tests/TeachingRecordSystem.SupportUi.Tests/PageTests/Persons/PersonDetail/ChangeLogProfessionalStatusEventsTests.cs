...
public class ChangeLogProfessionalStatusEventsTests(HostFixture hostFixture) : TestBase(hostFixture)
{
    ...
    public async Task ProfessionalStatusUpdatedEvent_RendersExpectedContent()
    {
        ...
        var route = oldRoute;
        var subject = (await ReferenceDataCache.GetTrainingSubjectsAsync()).Where(x => x.TrainingSubjectId != oldSubject.TrainingSubjectId).Where(s => s.Name.IndexOf('\'') == -1).SingleRandom();
    ...
    public async Task ProfessionalStatusUpdatedEvent_StatusChangedToHolds_PersonQtsChanged_RendersExpectedContent()
    {
        ...
        var route = (await ReferenceDataCache.GetRouteToProfessionalStatusTypesAsync())
            ...
            .SingleRandom();
        var status = RouteToProfessionalStatusStatus.Holds;
        var subject = (await ReferenceDataCache.GetTrainingSubjectsAsync()).Where(s => s.Name.IndexOf('\'') == -1).SingleRandom();
        var trainingProvider = (await ReferenceDataCache.GetTrainingProvidersAsync()).SingleRandom();
        var degreeType = (await ReferenceDataCache.GetDegreeTypesAsync()).SingleRandom();
        var country = (await ReferenceDataCache.GetTrainingCountriesAsync()).SingleRandom();
        var ageRange = TrainingAgeSpecialismType.KeyStage1;
