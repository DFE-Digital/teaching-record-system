...
public class IndexTests : SetStatusTestBase
{
    ...
    public async Task Get_PageTitle_CaptionIsExpected(PersonStatus targetStatus)
    {
        // Arrange
        var person = await CreatePersonToBecome(targetStatus, p => p
            .WithFirstName("Lily")
            .WithMiddleName("The")
            .WithLastName("Pink"));
    ...
    [InlineData(PersonStatus.Active)]
    [InlineData(PersonStatus.Deactivated)]
    public async Task Get_ContinueAndCancelButtons_ExistOnPage(PersonStatus targetStatus)
    {
        // Arrange
        var person = await CreatePersonToBecome(targetStatus);
    ...
    public async Task Get_WithPreviouslyStoredChoices_ShowsChoices(PersonStatus targetStatus)
    {
        ...
        Assert.Equal(evidenceFileId.ToString(), doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileId)));
        Assert.Equal("evidence.jpg", doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileName)));
        Assert.Equal("1.2 KB", doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileSizeDescription)));
        Assert.Equal(expectedFileUrl, doc.GetHiddenInputValue(nameof(IndexModel.UploadedEvidenceFileUrl)));
    }
    ...
    public async Task Post_NoChoicesAreEntered_ReturnsErrors(PersonStatus targetStatus)
    {
        ...
        if (targetStatus == PersonStatus.Deactivated)
        {
            await AssertEx.HtmlResponseHasErrorAsync(response, nameof(IndexModel.DeactivateReason), "Select a reason");
        }
        else
        {
            await AssertEx.HtmlResponseHasErrorAsync(response, nameof(IndexModel.ReactivateReason), "Select a reason");
        }
        await AssertEx.HtmlResponseHasErrorAsync(response, nameof(IndexModel.UploadEvidence), "Select yes if you want to upload evidence");
    }
    ...
    public async Task Post_AnotherReason_NoDetailAdded_ReturnsError(PersonStatus targetStatus)
    {
        ...
        if (targetStatus == PersonStatus.Deactivated)
        {
            await AssertEx.HtmlResponseHasErrorAsync(response, nameof(IndexModel.DeactivateReasonDetail), "Enter a reason");
        }
        else
        {
            await AssertEx.HtmlResponseHasErrorAsync(response, nameof(IndexModel.ReactivateReasonDetail), "Enter a reason");
        }
    }
    ...
    public async Task Post_FileUploadYes_NoFileUploaded_ReturnsError(PersonStatus targetStatus)
    {
        ...
        // Assert
        await AssertEx.HtmlResponseHasErrorAsync(response, nameof(IndexModel.EvidenceFile), "Select a file");
    }
    ...
    public async Task Post_UploadEvidenceSetToYes_ButEvidenceFileIsInvalidType_RendersError(PersonStatus targetStatus)
    {
        ...
        await AssertEx.HtmlResponseHasErrorAsync(response, nameof(IndexModel.EvidenceFile), "The selected file must be a BMP, CSV, DOC, DOCX, EML, JPEG, JPG, MBOX, MSG, ODS, ODT, PDF, PNG, TIF, TXT, XLS or XLSX");
    }
    ...
    public async Task Post_UploadEvidenceSetToYes_AndEvidenceFileIsSelected_ButOtherFieldsInvalid_ShowsUploadedFile(PersonStatus targetStatus)
    {
        ...
        Assert.Equal(evidenceFileId.ToString(), doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileId)));
        Assert.Equal("validfile.png", doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileName)));
        Assert.Equal("1.2 KB", doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileSizeDescription)));
        Assert.Equal(expectedFileUrl, doc.GetHiddenInputValue(nameof(IndexModel.UploadedEvidenceFileUrl)));
    }
    ...
    public async Task Post_UploadEvidenceSetToYes_AndEvidenceFilePreviouslyUploaded_ButOtherFieldsInvalid_RemembersUploadedFile(PersonStatus targetStatus)
    {
        ...
        Assert.Equal(evidenceFileId.ToString(), doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileId)));
        Assert.Equal("testfile.jpg", doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileName)));
        Assert.Equal("3 KB", doc.GetHiddenInputValue(nameof(IndexModel.EvidenceFileSizeDescription)));
        Assert.Equal("http://test.com/file", doc.GetHiddenInputValue(nameof(IndexModel.UploadedEvidenceFileUrl)));
    }
    ...
    private string GetRequestPath(TestData.CreatePersonResult person, PersonStatus targetStatus, JourneyInstance<SetStatusState> journeyInstance) =>
        $"/persons/{person.PersonId}/set-status/{targetStatus}?{journeyInstance.GetUniqueIdQueryParameter()}";
