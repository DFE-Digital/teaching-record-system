...
public class CheckAnswersTests : SetStatusTestBase
{
    ...
    // TODO: Common page tests
    // TODO: Precondition tests on request (BadRequest)
    // TODO: Rename these tests
    // TODO: Rename Index page?
    // TODO: extra BL from ticket:
    //   * A record cannot be reactivated if it was deactivated as part of a merge, so we need to account for this.
    ...
    [InlineData(PersonStatus.Active)]
    [InlineData(PersonStatus.Deactivated)]
    public async Task Get_ConfirmAndCancelButtons_ExistOnPage(PersonStatus targetStatus)
    {
        // Arrange
        ...
        Assert.Collection(buttons,
            b => Assert.Equal(targetStatus == PersonStatus.Deactivated
                ? "Confirm and deactivate record"
                : "Confirm and reactivate record", b.TrimmedText()),
    ...
    [InlineData(PersonStatus.Active)]
    [InlineData(PersonStatus.Deactivated)]
    public async Task Get_WhenOtherDetailsFieldChanged_ShowsDetailsChangeReasonAndEvidenceFile_AsExpected(PersonStatus targetStatus)
    {
        // Arrange
    ...
    [InlineData(PersonStatus.Active)]
    [InlineData(PersonStatus.Deactivated)]
    public async Task Get_WhenOtherDetailsFieldChanged_ShowsMissingAdditionalDetailAndEvidenceFile_AsNotProvided(PersonStatus targetStatus)
    {
        // Arrange
        var person = await CreatePersonToBecome(targetStatus);
    ...
    [InlineData(PersonStatus.Active)]
    [InlineData(PersonStatus.Deactivated)]
    public async Task Post_Confirm_UpdatesPersonEditDetailsCreatesEventCompletesJourneyAndRedirectsWithFlashMessage(PersonStatus targetStatus)
    {
        // Arrange
        var evidenceFileId = Guid.NewGuid();
