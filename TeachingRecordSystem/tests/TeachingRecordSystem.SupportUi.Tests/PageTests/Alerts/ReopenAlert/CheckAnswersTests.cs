...
public class CheckAnswersTests(HostFixture hostFixture) : ReopenAlertTestBase(hostFixture)
{
    ...
    public async Task Get_WithValidJourneyState_ReturnsOk(bool populateOptional)
    {
        ...
        Assert.Equal(populateOptional ? journeyInstance.State.ChangeReasonDetail : UiDefaults.EmptyDisplayContent, doc.GetSummaryListValueByKey("Reason details"));
