...
public class CheckAnswersTests(HostFixture hostFixture) : StartDateTestBase(hostFixture)
{
    [Before(Test)]
    ...
    public async Task Get_ValidJourneyState_ReturnsOk(bool populateOptional)
    {
        ...
        Assert.Equal(alert.StartDate!.Value.ToString(UiDefaults.DateOnlyDisplayFormat), doc.GetSummaryListValueForKey("Previous start date"));
        Assert.Equal(journeyInstance.State.ChangeReason!.Value.GetDisplayName(), doc.GetSummaryListValueForKey("Reason"));
        Assert.Equal(populateOptional ? journeyInstance.State.ChangeReasonDetail : UiDefaults.EmptyDisplayContent, doc.GetSummaryListValueForKey("Additional information"));
    ...
    public async Task PersonIsDeactivated_ReturnsBadRequest(HttpMethod httpMethod)
    {
        ...
        var request = new HttpRequestMessage(httpMethod, $"/alerts/{alert.AlertId}/start-date/check-answers?{journeyInstance.GetUniqueIdQueryParameter()}");
