...
public class StartAndEndDateTests(HostFixture hostFixture) : TestBase(hostFixture)
{
    ...
    public async Task Post_WhenStartAndEndDateIsEntered_SavesDateAndRedirectsToNextPage()
    {
        ...
        var endDate = startDate.AddMonths(1);
        var route = (await ReferenceDataCache.GetRouteToProfessionalStatusTypesAsync())
            .Where(r => r.TrainingStartAndEndDatesRequired == FieldRequirement.Mandatory && r.TrainingProviderRequired == FieldRequirement.Mandatory)
            .RandomOne();
        var status = ProfessionalStatusStatusRegistry.All
            .Where(s => s.TrainingStartAndEndDateRequired == FieldRequirement.Mandatory && s.TrainingProviderRequired == FieldRequirement.Optional && s.HoldsFromRequired == FieldRequirement.NotApplicable)
            .RandomOne()
            .Value;
        var person = await TestData.CreatePersonAsync();
        var addRouteState = new AddRouteStateBuilder()
