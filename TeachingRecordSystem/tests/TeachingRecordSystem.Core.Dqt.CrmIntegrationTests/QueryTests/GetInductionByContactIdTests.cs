...
public class GetInductionByContactIdTests : IAsyncLifetime
{
    ...
    public async Task WhenCalled_WithContactIdForNonExistentContact_ReturnsNullInductionAndInductionPeriods()
    {
        ...
        // Act
        var result = await _crmQueryDispatcher.ExecuteQuery(new GetInductionByContactIdQuery(nonExistentContactId));
    ...
    public async Task WhenCalled_WithContactWithInduction_ReturnsResultWithInductionAndNullInductionPeriods()
    {
        ...
        var createPersonResult = await _dataScope.TestData.CreatePerson(x =>
        {
            ...
            x.WithInduction(inductionStatus: dfeta_InductionStatus.Pass, inductionExemptionReason: null, null, null, null);
        });

        // Act
        var result = await _crmQueryDispatcher.ExecuteQuery(new GetInductionByContactIdQuery(createPersonResult.ContactId));
    ...
    public async Task WhenCalled_WithInductionAndInductionPeriod_ReturnsExpectedResults()
    {
        ...
        {
            x.WithQts(new DateOnly(2021, 01, 1));
            x.WithInduction(inductionStatus: dfeta_InductionStatus.Pass, inductionExemptionReason: null, startDate: new DateOnly(2021, 01, 01), endDate: new DateOnly(2022, 01, 01), appropriateBodyOrgId: establishment1.AccountId);
        });
