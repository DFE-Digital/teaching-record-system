...
public class ApplicationManager : OpenIddictApplicationManager<OpenIddictEntityFrameworkCoreApplication<Guid>>
{
    ...
    public override ValueTask<bool> ValidateRedirectUriAsync(OpenIddictEntityFrameworkCoreApplication<Guid> application, string uri, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrEmpty(application?.RedirectUris) || string.IsNullOrEmpty(uri))
        {
            return ValueTask.FromResult(false);
        }
