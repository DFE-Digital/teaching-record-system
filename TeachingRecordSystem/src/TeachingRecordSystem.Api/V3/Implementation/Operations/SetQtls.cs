    ...
    ICrmQueryDispatcher crmQueryDispatcher)
{
    public async Task<ApiResult<QtlsResult>> HandleAsync(SetQtlsCommand command)
    {
        ...
        var qtlsRouteId = PostgresModels.RouteToProfessionalStatus.QtlsAndSetMembershipId;
        var qtlsQualifications = person.Qualifications!.OfType<PostgresModels.ProfessionalStatus>()
            .Where(p => p.RouteToProfessionalStatusId == qtlsRouteId)
            .ToArray();
        ...
        if (command.QtsDate is not null)
        {
            ...
            {
                var professionalStatus = PostgresModels.ProfessionalStatus.Create(
                    person,
                    allRoutes: await referenceDataCache.GetRoutesToProfessionalStatusAsync(activeOnly: false),
                    routeToProfessionalStatusId: qtlsRouteId,
