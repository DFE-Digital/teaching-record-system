    ...
    public async Task<ApiResult<SetRouteToProfessionalStatusResult>> HandleAsync(SetRouteToProfessionalStatusCommand command)
    {
        ...
        else
        {
            route = RouteToProfessionalStatus.Create(
                person,
                allRouteTypes: await referenceDataCache.GetRouteToProfessionalStatusTypesAsync(),
                command.RouteToProfessionalStatusTypeId,
                command.Status,
                command.HoldsFrom,
                command.TrainingStartDate,
                command.TrainingEndDate,
                subjectIds.ToArray(),
                command.TrainingAgeSpecialism?.Type,
                command.TrainingAgeSpecialism?.From,
                command.TrainingAgeSpecialism?.To,
                command.TrainingCountryReference,
                trainingProviderId,
                degreeTypeId,
                command.IsExemptFromInduction,
                currentUserId,
                clock.UtcNow,
                changeReason: null,
                changeReasonDetail: null,
