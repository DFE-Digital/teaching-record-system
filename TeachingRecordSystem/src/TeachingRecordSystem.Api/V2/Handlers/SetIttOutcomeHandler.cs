...
public class SetIttOutcomeHandler : IRequestHandler<SetIttOutcomeRequest, SetIttOutcomeResponse>
{
    ...
    public async Task<SetIttOutcomeResponse> Handle(SetIttOutcomeRequest request, CancellationToken cancellationToken)
    {
        ...
        var result = await _dataverseAdapter.SetIttResultForTeacherAsync(
            teacherId,
    ...
    private async Task<Contact[]> GetTeacherByTrnDobOrSlugIdAsync(string trn, DateOnly? dob, string slugId)
    {
        if (!string.IsNullOrEmpty(slugId))
        {
            var contacts = await _dataverseAdapter.GetTeachersBySlugIdAndTrnAsync(slugId, trn, columnNames: Array.Empty<string>(), true);
