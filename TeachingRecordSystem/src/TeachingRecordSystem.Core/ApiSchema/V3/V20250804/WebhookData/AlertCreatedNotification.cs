...
public class AlertCreatedNotificationMapper(PersonInfoCache personInfoCache, ReferenceDataCache referenceDataCache) :
    IEventMapper<LegacyEvents.AlertCreatedEvent, AlertCreatedNotification>
{
    public async Task<AlertCreatedNotification?> MapEventAsync(LegacyEvents.AlertCreatedEvent @event)
    {
