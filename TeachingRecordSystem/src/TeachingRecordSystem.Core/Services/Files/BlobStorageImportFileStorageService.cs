...
public class BlobStorageImportFileStorageService(BlobServiceClient blobServiceClient) : IImportFileStorageService
{
    public async Task<string[]> GetFileNamesAsync(string containerName, string folderName, CancellationToken cancellationToken)
    {
        var blobContainerClient = blobServiceClient.GetBlobContainerClient(containerName);
        var fileNames = await GetFileNamesAsync(blobContainerClient, folderName, true, cancellationToken);
        return fileNames.OrderBy(f => f).ToArray();
    }

    public async Task<Stream> GetFileAsync(string containerName, string fileName, CancellationToken cancellationToken)
    {
        var blobContainerClient = blobServiceClient.GetBlobContainerClient(containerName);
    ...
    public async Task<Stream> WriteFileAsync(string containerName, string fileName, CancellationToken cancellationToken)
    {
        var blobContainerClient = blobServiceClient.GetBlobContainerClient(containerName);
        var blobClient = blobContainerClient.GetBlobClient(fileName);
    ...
    public async Task MoveFileAsync(string containerName, string fileName, string targetFolderName, CancellationToken cancellationToken)
    {
        var blobContainerClient = blobServiceClient.GetBlobContainerClient(containerName);
        ...
        if (await sourceBlobClient.ExistsAsync(cancellationToken))
        {
            ...
            var targetFileName = $"{targetFolderName}/{fileNameWithoutFolder}";
