        ...
        async Task<IEnumerable<IttQtsMapResult>> MapContactIttAndQtsAsync(Contact contact)
        {
            ...
            foreach (var qts in contactQts.OrderBy(q => q.dfeta_QTSDate is not null).ThenBy(q => q.dfeta_DateofRecognition is not null).ThenBy(q => q.dfeta_EYTSDate is not null).ThenBy(q => q.dfeta_EarlyYearsStatusId?.Id == earlyYearsProfessionalStatus.Id))
            {
                ...
                foreach (var isEy in new[] { false, true })
                {
                    ...
                    // Very specific override for a contact which has multiple potential ITT records but we know which one to use
                    if (contactId == Guid.Parse("2cbbe3f2-05af-e311-b8ed-005056822391"))
                    {
                        if (isEy)
                        {
                            itt = matchingItt
                                .Where(i => i.Id == Guid.Parse("69d3722c-bb91-e911-a958-000d3a2aa275"))
                                .FirstOrDefault();
                        }
                        else
                        {
                            itt = matchingItt
                                .Where(i => i.Id == Guid.Parse("3f5b2a42-35b0-e311-b8ed-005056822391"))
                                .FirstOrDefault();
                        }
                    }
                    else
                    {
                        ...
                        itt = matchingItt
                            .OrderBy(itt => (itt.dfeta_Result == dfeta_ITTResult.Pass || itt.dfeta_Result == dfeta_ITTResult.Approved) && ((!isEy && (qts.dfeta_QTSDate is not null || qts.dfeta_DateofRecognition is not null)) || (isEy && ((qts.dfeta_EYTSDate is not null)))) ? 0 : 1)
                            .ThenBy(itt => (eyStatus == earlyYearsProfessionalStatus && itt.dfeta_AgeRangeFrom == dfeta_AgeRange._00 && itt.dfeta_AgeRangeTo == dfeta_AgeRange._05) ? 0 : 1)
                            .ThenBy(itt => (eyStatus == earlyYearsProfessionalStatus && itt.dfeta_AgeRangeFrom is null && itt.dfeta_AgeRangeTo is null) ? 0 : 1)
            ...
            foreach (var itt in contactItt)
            {
                ...
                if (itt.dfeta_Result is dfeta_ITTResult.Pass or dfeta_ITTResult.Approved)
                {
                    ...
                    {
                        failedReason = IttQtsMapResultFailedReason.PotentialDuplicateIttRecords;
                    }
