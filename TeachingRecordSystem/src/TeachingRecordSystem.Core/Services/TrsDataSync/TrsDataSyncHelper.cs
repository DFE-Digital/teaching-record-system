        ...
        async Task<IEnumerable<IttQtsMapResult>> MapContactIttAndQtsAsync(Contact contact)
        {
            ...
            foreach (var qts in contactQts)
            {
                ...
                foreach (var isEy in new[] { true, false })
                {
                    ...
                    // Ordering use the one with a programme type first as createdon is unreliable due to it being reset from previous data migrations
                    // TODO sort the order we process QTS to use least dodgy looking ones first
                    itt = matchingItt.OrderBy(itt => itt.dfeta_ProgrammeType is not null ? 0 : 1).ThenBy(itt => itt.GetAttributeValue<DateTime>("createdon")).FirstOrDefault();
