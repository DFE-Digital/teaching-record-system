    ...
    IClock clock)
{
        ...
        bool createMigratedEvent)
    {
        ...
        EventBase MapMigratedEvent(EntityVersionInfo<dfeta_qualification> snapshot)
        {
            ...
            return new MandatoryQualificationMigratedEvent()
            {
                ...
                RaisedBy = EventModels.RaisedByUserInfo.FromUserId(DataStore.Postgres.Models.SystemUser.SystemUserId),
                PersonId = snapshot.Entity.dfeta_PersonId.Id,
    ...
    private async Task<int> SyncEvents(IReadOnlyCollection<EventBase> events, NpgsqlTransaction transaction, CancellationToken cancellationToken)
    {
        ...
        var insertStatement =
            $"""
            UPDATE {tableName} e
            SET event_id = t.event_id, payload = t.payload, created = t.created, inserted = t.inserted, published = false
            FROM {tempTableName} AS t
            WHERE e.event_id = t.event_id OR (t.key IS NOT NULL AND e.key = t.key);
