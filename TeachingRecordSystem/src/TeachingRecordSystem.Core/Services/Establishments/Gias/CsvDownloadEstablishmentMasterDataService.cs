    ...
    public async IAsyncEnumerable<Establishment> GetEstablishmentsAsync()
    {
        ...
        await foreach (var item in csv.GetRecordsAsync<EstablishmentCsvRow>())
        {
            yield return new Establishment
            {
                ...
                PhaseOfEducationCode = item.PhaseOfEducationCode == null ? null : int.Parse(item.PhaseOfEducationCode),
                PhaseOfEducationName = item.PhaseOfEducationName,
                NumberOfPupils = item.NumberOfPupils == null ? null : int.Parse(item.NumberOfPupils),
                FreeSchoolMealsPercentage = item.FreeSchoolMealsPercentage == null ? null : decimal.Parse(item.FreeSchoolMealsPercentage),
                Street = item.Street,
