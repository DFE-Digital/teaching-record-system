...
public class PreviousNameMapping : IEntityTypeConfiguration<PreviousName>
{
    public void Configure(EntityTypeBuilder<PreviousName> builder)
    {
        ...
        builder.HasIndex(p => p.DqtFirstNamePreviousNameId).IsUnique();
        builder.HasIndex(p => p.DqtMiddleNamePreviousNameId).IsUnique();
        builder.HasIndex(p => p.DqtLastNamePreviousNameId).IsUnique();
