...
public class GetQualificationByIdHandler : ICrmQueryHandler<GetQualificationByIdQuery, dfeta_qualification?>
{
    public async Task<dfeta_qualification?> Execute(GetQualificationByIdQuery query, IOrganizationServiceAsync organizationService)
    {
        var filter = new FilterExpression();
        filter.AddCondition(dfeta_qualification.PrimaryIdAttribute, ConditionOperator.Equal, query.QualificationId);
        ...
        return result.EntityCollection.Entities.Select(entity => entity.ToEntity<dfeta_qualification>()).SingleOrDefault();
    }
}
