...
public static class IttQualificationExtensions
{
    ...
    {
        if (ittQualification.TryConvertToTrsDegreeTypeId(out var result))
        {
            throw new ArgumentException($"{ittQualification.dfeta_Value} cannot be converted to {nameof(DegreeType)}.", nameof(ittQualification));
        }

        return result;
    }

    public static bool TryConvertToTrsDegreeTypeId(this dfeta_ittqualification ittQualification, out Guid? result)
    {
        var mapped = ittQualification.dfeta_Value switch
        {
