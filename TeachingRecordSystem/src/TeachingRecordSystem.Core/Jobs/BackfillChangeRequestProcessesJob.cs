...
public class BackfillChangeRequestProcessesJob(TrsDbContext dbContext)
{
    public async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        var results = await dbContext.Database.SqlQueryRaw<Result>(
            ...
            returning process_event_id, process_id, payload->'SupportTask'->'SupportTaskType' support_task_type, payload->'SupportTask'->'Data'->'EmailAddress' email_address, person_ids, created_on;
            """).ToListAsync(cancellationToken);

        foreach (var resultsForPerson in results.GroupBy(r => r.person_ids.Single()))
        {
            var emailAddress = resultsForPerson.First().email_address;

            var emailsForAddress = await dbContext.Emails.Where(e => e.EmailAddress == emailAddress).ToListAsync(cancellationToken);

            foreach (var e in resultsForPerson)
            {
                //var @event = e.payload.Deserialize<IEvent>(IEvent.SerializerOptions)!;

                var templateId = e.support_task_type == SupportTaskType.ChangeDateOfBirthRequest
                ...
                var emailsForTemplate = emailsForAddress.Where(m => m.TemplateId == templateId).ToArray();

                var thisTaskEmail = emailsForTemplate.Where(m => m.SentOn > e.created_on).OrderBy(m => m.SentOn).First();

                if ((thisTaskEmail.SentOn!.Value - e.created_on).Seconds > 30)
                {
