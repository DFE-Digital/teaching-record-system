...
public class CapitaExportNewJob(BlobServiceClient blobServiceClient, ILogger<CapitaExportNewJob> logger, TrsDbContext dbContext, IClock clock)
{
        ...
        using (var csvWriter = new CsvWriter(streamWriter, new CsvConfiguration(CultureInfo.InvariantCulture)))
        {
            ...
            finally
            {
                ...
                if (jobMetaData != null)
                {
                    ...
                    {
                        {
                            LastRunDateKey, clock.UtcNow.ToString("o")
                        }
                    };
                }
                else
    ...
    public string GetNewPersonWithPreviousLastNameAsStringRow(Person person)
    {
        ...
        var previousName = nameChangeEvent?.OldPersonAttributes.LastName;
    ...
    {
        var persons = await dbContext.Persons.Where(x => x.CreatedOn > lastRunDate && x.Trn != null && x.CreatedByTps != true).ToListAsync();
        return persons;
    }
}
