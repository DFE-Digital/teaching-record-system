...
public class QtsImporter(ICrmQueryDispatcher crmQueryDispatcher, ReferenceDataCache cache, ILogger<QtsImporter> logger)
{
        ...
        foreach (var row in records)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var qualificationQuery = new CreateHeQualificationQuery()
                    {
                        Id = qualificationId,
                        ContactId = lookupData.PersonId.Value,
                    ...
                    //Qts
                    var qtsQuery = new CreateQtsRegistrationQuery()
                    {
                    ...
                    if (row.QtsStatus == "67")
                    {
                        ...
                        var induction = new CreateInductionQuery()
                        {
    ...
    {
        var results = await cache.GetHeQualificationsAsync();
    ...
    {
        var results = await cache.GetHeSubjectsAsync();
    ...
    {
        var results = await cache.GetTeacherStatusesAsync();
    ...
    {
        var results = await cache.GetIttQualificationsAsync();
    ...
    {
        var subject = await cache.GetIttSubjectAsync(subjectCode);
    ...
    {
        var country = await cache.GetCountryAsync(countryCode);
        if (country is null)
            return (EwcWalesMatchStatus.NoMatch, null);
