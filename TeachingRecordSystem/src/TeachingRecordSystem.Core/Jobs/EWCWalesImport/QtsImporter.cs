...
public class QtsImporter(ICrmQueryDispatcher crmQueryDispatcher, ReferenceDataCache cache, ILogger<QtsImporter> logger, TrsDbContext dbContext)
{
    ...
    public async Task<QtsImportLookupData> GetLookupDataAsync(EwcWalesQtsFileImportData row)
    {
        ...
        if (personId.HasValue)
        {
            ...
            hasActiveAlerts = dbContext.Alerts.Where(x => x.PersonId == personId.Value && x.IsOpen).Count() > 0;
        }
    ...
    {
        var subject = await cache.GetIttSubjectBySubjectCodeAsync(subjectCode);
    ...
    {
        var country = await cache.GetCountryByCountryCodeAsync(countryCode);
        if (country is null)
        {
            return (EwcWalesMatchStatus.NoMatch, null);
        }
