...
public class EditApiKeyLinkGenerator(LinkGenerator linkGenerator)
{
    public string Index(Guid apiKeyId) =>
        linkGenerator.GetRequiredPathByPage("/ApiKeys/Index", routeValues: new { apiKeyId });

    public string Expire(Guid apiKeyId) =>
        linkGenerator.GetRequiredPathByPage("/ApiKeys/Index", handler: "Expire", routeValues: new { apiKeyId });
}
